...
void connectMQTT() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    // Resolve hostname
    IPAddress ip;
    Serial.printf("MQTT[%d] resolving %s...", idx, MQTT_HOSTS[idx]);
    Serial.flush();
    ...
    Serial.printf("MQTT[%d] connecting (cid=%s)...", idx, cid.c_str());
    Serial.flush();
    ...
    Serial.printf(" rc=%d (%s)\n", mqtt.state(), MQTT_HOSTS[idx]);
    Serial.flush();
    delay(500);
  }
}
...
void readPhysicalSensors() {
  // DHT22 → T1 (Temp idx=0) and H1 (Humidity idx=15)
  ...
  // Potentiometer → V1 (Vibration idx=5)
  int potRaw = analogRead(POT_PIN);
  float vib = (potRaw / 4095.0) * 5.0;
  sensors[5].cur = roundf(vib * 100.0) / 100.0;

  // Photoresistor → L1 (Light idx=30)
  int ldrRaw = analogRead(LDR_PIN);
  float lux = (ldrRaw / 4095.0) * 1000.0;
  sensors[30].cur = roundf(lux * 100.0) / 100.0;

  // HC-SR04 → U1 (Ultrasonic idx=25)
  ...
  // Pushbutton → inject random fault on press
  bool btn = digitalRead(BTN_PIN);
  if (btn == LOW && lastBtnState == HIGH && millis() - lastBtnRead > 300) {
    lastBtnRead = millis();
    Serial.println("BTN fault inject");
    ...
    Serial.println("  injected 3 faults");
  }
  lastBtnState = btn;
}
...
void publishSensors() {
  ...
  bool ok = mqtt.publish(TOPIC_SENSORS, jsonBuf);
  if (mqttEverConnected) {
    Serial.printf("Pub %d (%uB) %s  queue=%d\n", TOTAL_SENSORS, len, ok ? "OK" : "FAIL", mqtt.getBufferSize());
  }
}
...
void loop() {
  ...
  // MQTT watchdog
  if (!mqtt.connected()) {
    if (millis() - lastConnAttempt > 5000) {
      connectMQTT();
    }
    delay(100);
    return;
